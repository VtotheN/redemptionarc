/**
 * FLASH DEEP VOL — single atomic TX:
 *   MarginFi flash USDC → Raydium CPMM addLiquidity (deepen pool)
 *   → swap USDC→HOP → swap HOP→USDC → removeLiquidity → T22 harvest → repay
 *
 * Revenue: LP fees on swap round-trip (we're 100% LP after deposit) +
 *          T22 transfer fees withheld on HOP movement.
 *
 * Pool:    EwoZHyXz48vZL1TwkpQoq31brW4G4NDwmMr1DMw6qqBV
 * Flash:   MarginFi USDC bank (2s37akK2...)
 *
 * ENV:
 *   SOLANA_RPC_URL    (required)
 *   DRY_RUN=true      (default; sim only)
 *   ALLOW_LIVE=true   (required to send)
 *   FLASH_USDC=10000  (USDC borrowed via flash)
 *   ADDLIQ_USDC=5000  (USDC to deposit as LP, must be < FLASH_USDC)
 *   SWAP_USDC=100     (USDC round-trip swap size)
 *   SLIPPAGE_BPS=50
 *   JITO_TIP_LAMPORTS=200000
 *   CU_LIMIT=600000
 *   CU_PRICE=10000
 *   ALT_ADDRESS=""    (optional ALT for versioned TX)
 */
...
async function main(): Promise<void> {
  ...
  // mintA in this pool is USDC (the script in add-hop-liquidity confirms baseIn for USDC).
  // We treat mintA = USDC, mintB = HOP.
  ...
  // AMM config: from poolKeys (loaded from getPoolInfoFromRpc)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  ...
  //    For estimation use current reserves (good enough for sim slippage).
  ...
  // After T22 withhold on HOP transfer OUT of pool vault (pool sends HOP to crank ATA),
  // T22 fee deducts from received: usable = hopOut - calcT22Fee(hopOut)
  const hopReceivedUsable = hopOutRaw - calcT22Fee(hopOutRaw);

  // For LEG2 input: we send `hopForSell` from crank to pool, T22 deducts from the transfer.
  ...
  // IX[7] swap HOP→USDC (use estimated usable HOP)
  ixs.push(makeSwapCpmmBaseInInstruction(
    RAYDIUM_CPMM_PROGRAM,
    crank.publicKey,
    authority,
  ...
  const tipUsdEst    = Number(jitoTipLamports) / 1e9 * 130; // assume ~$130 SOL
  const netEst       = swapLpFeeUsd + t22FeeUsd - tipUsdEst;

  console.log("=== FLASH DEEP VOL ===");
  ...
  console.log(`LP fee:     ~$${swapLpFeeUsd.toFixed(4)} (= $${swapUsdcUi} * 0.05% * 2)`);
  ...
  // ── Build TX (try legacy first; fall back to v0 + ALT if too large) ──
  ...
  // Try legacy
  const legacyTx = new Transaction({ recentBlockhash: blockhash, feePayer: crank.publicKey });
  legacyTx.add(...ixs);

  let useLegacy = true;
  try {
    legacyTx.partialSign(crank);
    const serialized = legacyTx.serialize({ requireAllSignatures: false });
    if (serialized.length > 1232) {
      useLegacy = false;
      console.log(`Legacy TX too large (${serialized.length}b), falling back to v0...`);
    }
  } catch (e) {
    useLegacy = false;
    console.log(`Legacy build failed (${(e as Error).message}), falling back to v0...`);
  }
  ...
  try {
    if (useLegacy) {
      if (dryRun || !allowLive) {
        ...
        const sig = await conn.sendTransaction(legacyTx, [crank], { skipPreflight: false });
